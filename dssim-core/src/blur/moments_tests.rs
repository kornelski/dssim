...
fn check<const NEED1: bool, const NEED2: bool>() {
    type H = unsafe fn(&[f32], &[f32], [&mut [f32]; 5]);
    type V = unsafe fn([&[f32]; 5], V5Edge, &mut [f32]);
    let mut hs: Vec<H> = vec![blur_h5_moments_row_base::<NEED1, NEED2>, blur_moments_row::<NEED1, NEED2>];
    let mut vs: Vec<V> = vec![v5_combine_row_base, v5_combine_row];
    #[cfg(target_arch = "x86_64")]
    {
        if crate::caps::has_avx2_fma() { hs.push(blur_h5_moments_row_avx2::<NEED1, NEED2>); vs.push(v5_combine_row_avx2); }
        if crate::caps::has_avx512() { hs.push(blur_h5_moments_row_avx512::<NEED1, NEED2>); vs.push(v5_combine_row_avx512); }
    }
    for n in [1,2,3,4,5,7,8,15,16,17,33,257] {
        ...
        {
            let mut expected=vec![f32::NAN;5*n];
            let mut rows=expected.chunks_mut(n);
            blur_h5_moments_row_base::<NEED1, NEED2>(&a,&b,std::array::from_fn(|_| rows.next().unwrap()));
            for kernel in &hs {
