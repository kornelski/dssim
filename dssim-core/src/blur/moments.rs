...
/// The const parameters select uncached inputs; their mean/square outputs are written,
/// along with the cross product which is always needed. Other outputs are untouched.
///
...
/// `blur5_inner_inline`.
#[inline(always)]
fn blur_h5_moments_row_inline<const NEED1: bool, const NEED2: bool>(
    r1: &[f32],
    r2: &[f32],
    rows: [&mut [f32]; 5],
) {
    ...
    if NEED1 { h5_edges(r1, r2, |a, _b, i| a[i], rows[0]); }
    if NEED2 { h5_edges(r1, r2, |_a, b, i| b[i], rows[1]); }
    if NEED1 { h5_edges(r1, r2, |a, _b, i| a[i] * a[i], rows[2]); }
    if NEED2 { h5_edges(r1, r2, |_a, b, i| b[i] * b[i], rows[3]); }
    ...
    if inner > 0 {
        ...
        for k in 0..inner {
            ...
            if NEED1 { d0[k] = (x0 + x4) * K5_OUTER + (x1 + x3) * K5_INNER + x2 * K5_MID; }
            if NEED2 { d1[k] = (y0 + y4) * K5_OUTER + (y1 + y3) * K5_INNER + y2 * K5_MID; }
            let (p0, p1, p2, p3, p4) = (x0 * x0, x1 * x1, x2 * x2, x3 * x3, x4 * x4);
            if NEED1 { d2[k] = (p0 + p4) * K5_OUTER + (p1 + p3) * K5_INNER + p2 * K5_MID; }
            let (q0, q1, q2, q3, q4) = (y0 * y0, y1 * y1, y2 * y2, y3 * y3, y4 * y4);
            if NEED2 { d3[k] = (q0 + q4) * K5_OUTER + (q1 + q3) * K5_INNER + q2 * K5_MID; }
...
#[inline(never)]
fn blur_h5_moments_row_base<const NEED1: bool, const NEED2: bool>(r1: &[f32], r2: &[f32], rows: [&mut [f32]; 5]) {
    blur_h5_moments_row_inline::<NEED1, NEED2>(r1, r2, rows);
}
...
#[inline(never)]
#[target_feature(enable = "avx2,fma")]
fn blur_h5_moments_row_avx2<const NEED1: bool, const NEED2: bool>(r1: &[f32], r2: &[f32], rows: [&mut [f32]; 5]) {
    blur_h5_moments_row_inline::<NEED1, NEED2>(r1, r2, rows);
}
...
#[inline(never)]
#[target_feature(enable = "avx2,fma,avx512f,avx512bw,avx512dq,avx512vl")]
fn blur_h5_moments_row_avx512<const NEED1: bool, const NEED2: bool>(r1: &[f32], r2: &[f32], rows: [&mut [f32]; 5]) {
    blur_h5_moments_row_inline::<NEED1, NEED2>(r1, r2, rows);
}
...
pub fn blur_moments_row<const NEED1: bool, const NEED2: bool>(r1: &[f32], r2: &[f32], rows: [&mut [f32]; 5]) {
    #[cfg(target_arch = "x86_64")]
    if crate::caps::has_avx512() {
        // SAFETY: has_avx512() confirmed AVX2/FMA and AVX-512 F/BW/DQ/VL.
        unsafe { blur_h5_moments_row_avx512::<NEED1, NEED2>(r1, r2, rows) };
        return;
    }
    #[cfg(target_arch = "x86_64")]
    if crate::caps::has_avx2_fma() {
        // SAFETY: has_avx2_fma() confirmed AVX2+FMA support.
        unsafe { blur_h5_moments_row_avx2::<NEED1, NEED2>(r1, r2, rows) };
        return;
    }
    blur_h5_moments_row_base::<NEED1, NEED2>(r1, r2, rows);
}
...
pub fn blur_moments_v5_row<const NEED1: bool, const NEED2: bool>(
    taps: [[&[f32]; 5]; 5],
    edge: V5Edge,
    mut out: [&mut [f32]; 5],
) {
    for (p, o) in out.iter_mut().enumerate() {
        if p == 4 || [NEED1, NEED2][p % 2] { v5_combine_row(taps[p], edge, o); }
    }
}
